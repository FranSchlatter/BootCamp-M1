...
LinkedList.prototype.add = function (data) {
  ...
  // bucle infinito en el primer node? 
...
LinkedList.prototype.search = function (x) {
  ...
  if (typeof(x) !== 'function'){
    ...
    while (x(current.value) !== true && current.next !== null){
      current = current.next;
    }
    if (x(current.value) === true){
      return current.value;
    } else {
      return false;
    }
...
function HashTable() {
  let table = [];
  table.length = 35;
	this.numBuckets = table.length;
};

HashTable.prototype.hash = function (key) {
  let pos = 0;
  if(typeof(key) !== "string") throw new TypeError("no es un string");
  for (let i=0;i<key.length;i++) pos = pos + key.charCodeAt(i);
  return pos%35;
};
