...
LinkedList.prototype.remove = function () {
  let current = this.head;
  if (!current) return null; // current === null 
...
LinkedList.prototype.search = function (x) {
  ...
  if (typeof(x) !== 'function'){
    while (current.value !== x && current.next){ // current.next !== null
      current = current.next;
    };
    if (current.value === x){
      return current.value;
    ...
    while (!x(current.value) && current.next){ // x(current.value) !== true && current.next !== null 
      current = current.next;
    };
    if (x(current.value)){ // x(current.value) === true
      return current.value;
    } else {
      return false;
    };
...
HashTable.prototype.hash = function (key) {
  let pos = 0;
  if(typeof(key) !== "string") throw new TypeError("no es un string");
  for (let i=0;i<key.length;i++) pos += key.charCodeAt(i);
  return pos%this.numBuckets;
};

HashTable.prototype.set = function(key, value) {
  let pos = this.hash(key);
  if (this[pos] === undefined) this[pos] = {};
  this[pos][key] = value;
};
...
HashTable.prototype.hasKey = function(key) {
  let pos = this.hash(key);
  return this[pos].hasOwnProperty(key);
};

// Antes del code-rewiew

// HashTable.prototype.set = function(key, value) {
//   let pos = this.hash(key);
//   if (this[pos] === undefined || this[pos][key] !== undefined){
//     this[pos] = {[key] : value};
//   } else {
//     Object.defineProperty(this[pos], key, {value: value});
//   };
// };
